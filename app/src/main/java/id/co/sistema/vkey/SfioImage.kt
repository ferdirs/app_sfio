...
class SfioImage : AppCompatActivity(), VosWrapper.Callback , VGExceptionHandler {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bt_dec = findViewById(R.id.bt_imgdec)
    ...
    private fun pickImg(){
        ...
        intent.type = "image/*"
        val ch = Intent.createChooser(intent , "upload gambar")
        launchGallery.launch(ch)
    ...
    private val launchGallery = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
        ...
        if (result.resultCode == RESULT_OK){
            val img_sel:Uri = result.data?.data as Uri
            val file = uriToFile(img_sel , this@SfioImage)
            getFile = file
            Log.d("lokasi", ": $imgURL")
        }
    }
    ...
    private fun uriToFile(selectImg: Uri , context: Context):File{
        ...
        imgURL = file.absolutePath

        val inputStream = contentResolver.openInputStream(selectImg) as InputStream
        val outputStream: OutputStream = java.io.FileOutputStream(file)
